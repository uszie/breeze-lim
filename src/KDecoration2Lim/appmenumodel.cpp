...
//    m_tasksModel->setFilterByScreen(true);
    connect(m_tasksModel, &TaskManager::TasksModel::destroyed, this, []() {
//        qDebug() << "AppMenuModel::AppMenuModel -> m_tasksModel destroyed";
    });
...
void AppMenuModel::onActiveWindowChanged()
{
    ...
    const QString appName = m_tasksModel->data(activeTaskIndex, TaskManager::AbstractTasksModel::AppName).toString();

    int rowCount= m_tasksModel->rowCount();
//qDebug() << "KDECORATION BLABLA total tasks =" << m_tasksModel->rowCount() << "AppName =" << appName << "objectPath =" << objectPath << "serviceName = " << serviceName << "activeTaskIndex=" << activeTaskIndex;
    for (int i = 0; i < m_tasksModel->rowCount(); i++) {
        const auto winIds = m_tasksModel->data(m_tasksModel->index(i, 0), TaskManager::AbstractTasksModel::WinIdList);
//        qDebug() << "BLABLA" << i << winIds;
    }
